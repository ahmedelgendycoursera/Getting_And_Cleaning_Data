...
The variables included in the data analysis are as follows:
subject 
activity 
tBodyAcc-mean()-X
tBodyAcc-mean()-Y
...
Data was collected from several files. The subjects, the activity, the mean, and the standard deviation were extracted in a separate data frame. Then, the mean is computed per subject-activity pair.
